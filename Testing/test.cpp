...
////    pipeline =
////            gst_parse_launch
////                    (" --gst-debug-level=5 playbin uri=file:///home/drones/CLionProjects/rtsp_streamer/Testing/dji.h264 ",
////                     NULL);
////
////    /* Start playing */
////    int a = gst_element_set_state (pipeline, GST_STATE_PLAYING);
////
...
//    next = static_cast<GstElement *>(g_queue_pop_head(&effects));
//    if (next == NULL) {
...
//    gst_pad_add_probe(srcpad, static_cast<GstPadProbeType>(GST_PAD_PROBE_TYPE_BLOCK |
...
//
//    effect = static_cast<GstElement *>(g_queue_pop_head(&effects));
//    cur_effect = effect;
//
...
main(int argc, char **argv) {
    ...
    for (round = 0; round < MAX_ROUND; round++) {
        ...
        if (round == 0){
            int aa = gst_element_set_state(pipe, GST_STATE_PLAYING);
