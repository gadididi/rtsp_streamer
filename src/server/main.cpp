...
static void media_prepared_cb(GstRTSPMedia *media) {
    std::cout << "media_prepared_cb" << std::endl;
    guint i, n_streams;
...
static void media_configure_cb(GstRTSPMediaFactory *factory, GstRTSPMedia *media) {
    ...
    int connected = g_signal_connect (media, "prepared", (GCallback) media_prepared_cb, factory);
    std::cout << "always greater than 0 for successful connections:  ";
...
//int main(int argc, char** argv)
//{
//    VideoCapture cap("udpsrc port=5000 ! queue ! application/x-rtp,encoding-name=H264 ! rtpjitterbuffer latency=0 ! rtph264depay ! avdec_h264 ! videoconvert ! autovideosink");
//
//    if( !cap.isOpened())
//    {
//        cout<<"Fail"<<endl;
